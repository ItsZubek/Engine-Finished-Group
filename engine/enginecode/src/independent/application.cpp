...
#pragma region TempSetup
		...
		float RenderVerts[4 * 4] =
		{
			0.f, 0.f, 0.f, 0.f,
			0.f, 145.f, 0.f, 1.0f,
			171.f, 145.f, 1.0f, 1.0f,
			171.f, 0.f, 1.0f, 0.f
		};

		float RenderVerts1[4 * 4] =
		{
			0.f, 0.f, 0.f, 0.f,
			0.f, 148.f, 0.f, 1.0f,
			254.f, 148.f, 1.0f, 1.0f,
			254.f, 0.f, 1.0f, 0.f
		};

		float RenderVerts2[4 * 4] =
		{
			0.f, 0.f, 0.f, 0.f,
			0.f, 235.f, 0.f, 1.0f,
			79.f, 235.f, 1.0f, 1.0f,
			79.f, 0.f, 1.0f, 0.f
		};
		
		unsigned int renderIndice[4] = { 0,1,2,3 };

		m_Renderer.reset(Renderer::createRenderer2D());
		VertexBufferLayout vbl = { { ShaderDataType::Float2 },{ ShaderDataType::Float2 } };
		m_RenderShader.reset(Shader::create("Assets/shaders/renderVert.glsl"));

		m_RenderVAO.reset(VertexArray::create());

		std::shared_ptr<VertexBuffer> renderVBO;
		renderVBO.reset(VertexBuffer::Create(RenderVerts, sizeof(RenderVerts), vbl));
		m_RenderVAO->setVertexBuffer(renderVBO);

		std::shared_ptr<IndexBuffer> renderIBO;
		renderIBO.reset(IndexBuffer::Create(renderIndice, 4));
		m_RenderVAO->setIndexBuffer(renderIBO);

		m_RenderTexture.reset(Texture::createFromFile("Assets/Tank.png"));

		m_renderMaterial.reset(Material::create(m_RenderShader, m_RenderVAO));

		
		m_RenderVAO1.reset(VertexArray::create());

		std::shared_ptr<VertexBuffer> renderVBO1;
		renderVBO1.reset(VertexBuffer::Create(RenderVerts1, sizeof(RenderVerts1), vbl));
		m_RenderVAO1->setVertexBuffer(renderVBO1);

		std::shared_ptr<IndexBuffer> renderIBO1;
		renderIBO1.reset(IndexBuffer::Create(renderIndice, 4));
		m_RenderVAO1->setIndexBuffer(renderIBO1);

		m_RenderTexture1.reset(Texture::createFromFile("Assets/Enemy.png"));

		m_renderMaterial1.reset(Material::create(m_RenderShader, m_RenderVAO1));


		m_RenderVAO2.reset(VertexArray::create());

		std::shared_ptr<VertexBuffer> renderVBO2;
		renderVBO2.reset(VertexBuffer::Create(RenderVerts2, sizeof(RenderVerts2), vbl));
		m_RenderVAO2->setVertexBuffer(renderVBO2);

		std::shared_ptr<IndexBuffer> renderIBO2;
		renderIBO2.reset(IndexBuffer::Create(renderIndice, 4));
		m_RenderVAO2->setIndexBuffer(renderIBO2);

		m_RenderTexture2.reset(Texture::createFromFile("Assets/Attack Beam.png"));

		m_renderMaterial2.reset(Material::create(m_RenderShader, m_RenderVAO2));
...
#pragma region TempDrawCode
			...
			glm::mat4 renderProjection = glm::ortho(0.f, 800.f, 600, 0.f);
			glm::mat4 renderView = glm::mat4(1.0f);
			glm::mat4 renderModel = glm::translate(glm::mat4(1.0f), glm::vec3(100.f, 100.f, 0.f));
			glm::mat4 renderModel1 = glm::translate(glm::mat4(1.0f), glm::vec3(300.f, 100.f, 0.f));
			glm::mat4 renderModel2 = glm::translate(glm::mat4(1.0f), glm::vec3(200.f, 100.f, 0.f));

			

			m_renderMaterial->setDataElement("u_projection", (void*)&renderProjection[0][0]);
			m_renderMaterial->setDataElement("u_view", (void*)&renderView[0][0]);
			m_renderMaterial->setDataElement("u_model", (void*)&renderModel[0][0]);
			m_renderMaterial->setDataElement("u_texData", (void*)&m_textureSlots);
			

			m_renderMaterial1->setDataElement("u_projection", (void*)&renderProjection[0][0]);
			m_renderMaterial1->setDataElement("u_view", (void*)&renderView[0][0]);
			m_renderMaterial1->setDataElement("u_model", (void*)&renderModel[0][0]);
			m_renderMaterial1->setDataElement("u_texData", (void*)&m_textureSlots);
			

			m_renderMaterial2->setDataElement("u_projection", (void*)&renderProjection[0][0]);
			m_renderMaterial2->setDataElement("u_view", (void*)&renderView[0][0]);
			m_renderMaterial2->setDataElement("u_model", (void*)&renderModel[0][0]);
			m_renderMaterial2->setDataElement("u_texData", (void*)&m_textureSlots);


			m_textureSlots = m_RenderTexture->getSlot();
			m_Renderer->submit(m_renderMaterial);

			m_textureSlots = m_RenderTexture1->getSlot();
			m_Renderer->submit(m_renderMaterial1);

			m_textureSlots = m_RenderTexture2->getSlot();
			m_Renderer->submit(m_renderMaterial2);
